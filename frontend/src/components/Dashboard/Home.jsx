...
const Home = () => {
  ...
  const renderAttendanceButtons = () => {
    ...
    return (
      <div className="flex items-center justify-center gap-4 bg-gray-100 border border-gray-300 rounded-lg p-3 shadow-sm">
        {showCheckIn && (
          <>
  ...
  return (
    <ProtectedRoute permission="dashboard_access">
      <div className="flex-col flex-grow p-4 font-rubik text-right space-y-6">
        {/*החתמה (כניסה/יציאה) */}
              ...
              const overdueCount =
                ...
                (user?.user_alert_dash === 1 &&
                  stats?.tasks_overdue
                    ...
                    .reduce((sum, t) => sum + t.overdue_count, 0));
              console.log("Tasks Overdue", stats?.tasks_overdue);
              return (
                overdueCount > 0 && (
                  <AlertBar
                    icon={
        ...
        <div className="w-full flex justify-center">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 max-w-7xl">
            ...
            <div className="bg-white rounded-xl shadow p-4 h-80">
              <LeadsByUserChart data={stats?.leads_by_user_status || []} />
            </div>
          </div>
        </div>
